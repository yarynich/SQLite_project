...
UnicodeString test = "DELETE FROM customers WHERE id="+IntToStr(selectedNodeIndex);
...
sqlite3_open("semis.db", &Database);
...
sqlite3_stmt *pStatement;
//Подготавливаем запрос
int result = sqlite3_prepare_v2(Database,"SELECT id, date, contractor FROM customers",-1,&pStatement,NULL);
...
/////Постановка запроса/////
UnicodeString test = "SELECT receipt_amount FROM customers WHERE id="+IntToStr(nodeData->index);
wchar_t *wc = test.t_str();
char c[100];
wcstombs(c, wc, wcslen(wc)+1);
/////////////////////////////
...
Label1->Caption = "Сумма поступления "+(UnicodeString)(char*)str;
sqlite3_finalize(pStatement);
}
//---------------------------------------------------------------------------
